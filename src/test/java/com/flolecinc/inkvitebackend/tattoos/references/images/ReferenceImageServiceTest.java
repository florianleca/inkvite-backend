...
class ReferenceImageServiceTest {
    ...
    void uploadImageFromUrl_nominal() throws IOException {
        // Given
        when(fileDownloader.getBytesFromFileUrl(WEB_IMAGE_URL)).thenReturn(MOCK_IMAGE_BYTES);
    ...
    void uploadImageFromUrl_imageUploadException() throws IOException {
        // Given
        when(fileDownloader.getBytesFromFileUrl(WEB_IMAGE_URL)).thenThrow(new IOException());

        // When & Then
        FileReaderException exception = assertThrows(FileReaderException.class, () -> referenceImageService.uploadImageFromUrl(WEB_IMAGE_URL));
